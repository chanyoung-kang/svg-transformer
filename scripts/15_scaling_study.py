"""
Scaling study for SVG decoder-only Transformer models.

This script trains multiple GPT-style model sizes on the same SVG token dataset.
Each model is trained for approximately 1 epoch, using the best peak learning
rate selected from the learning-rate sweep.

For each model, the script saves:
    1. final model checkpoint (.pt)
    2. final metrics row in outputs/scaling_results.csv
    3. intermediate training/validation losses in outputs/training_curves.csv

The CSV files are updated during training so partial results are preserved if
Colab disconnects or a larger model fails.
...
Outputs:
    ...
    outputs/checkpoints/scaling_<model_name>.pt
"""
...
# Same data/batch setup across model sizes
BATCH_SIZE = 32
BLOCK_SIZE = 256

# Best peak LR from LR sweep
MAX_LR = 8e-3
...
WEIGHT_DECAY = 0.1
GRAD_CLIP = 1.0
USE_AMP = True  # mixed precision on CUDA to reduce memory and improve speed
...
def get_lr(step: int, max_lr: float, max_iters: int, warmup_iters: int) -> float:
    ...
    Warmup:
        LR increases from near 0 to max_lr.

    Cosine decay:
        LR decreases from max_lr to min_lr.
    """
...
def save_results(results: list[dict]) -> None:
    """
    Save final per-model metrics collected so far.
    """
    ...
    print(f"\nSaved current scaling results to: {RESULTS_PATH}")


def save_curves(curve_rows: list[dict]) -> None:
    """
    Save training/validation loss curves collected so far.

    This is called after every evaluation point, so partial curve data is
    preserved if Colab disconnects.
    """
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    print(f"Saved current training curves to: {CURVES_PATH}")
...
def train_one_model(
    ...
    """
    Train one model configuration for approximately 1 epoch.
    """
    ...
    checkpoint_path = CHECKPOINT_DIR / f"scaling_{model_name}.pt"
...
def failed_result(model_cfg: dict, error: Exception) -> dict:
    """
    Create a CSV row for a failed model.
    """
    return {
        "model_name": model_cfg["model_name"],
        "status": "failed",
...
def main():
    ...
    for model_cfg in MODEL_CONFIGS:
        try:
            ...
            print("\n" + "!" * 80)
            print(f"Model failed: {model_cfg['model_name']}")
            print(repr(e))
            traceback.print_exc()
            print("!" * 80)
